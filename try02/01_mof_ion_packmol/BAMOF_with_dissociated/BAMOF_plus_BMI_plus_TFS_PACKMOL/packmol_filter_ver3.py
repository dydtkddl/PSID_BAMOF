...
def cell_matrix(a, b, c, alpha, beta, gamma):
    ...
    vx = np.cos(beta_r)
    vy = (np.cos(alpha_r) - np.cos(beta_r) * np.cos(gamma_r)) / np.sin(gamma_r)
...
def main():
    ...
    cell_mat = np.array(
      [  [  15.99943,  -0.55726,  0.08957],
        [  -6.51703,  14.61251, -0.05652],
        [  -4.63465,  -7.06644, 13.09461]]
    )
    print(cell_mat)
